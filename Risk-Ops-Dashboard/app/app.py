...
# --------------------------------------------------
st.set_page_config(
    page_title="Risk and Incident Analytics Dashboard",
    layout="wide",
...
def generate_insights(df):
    ...
    insights.append(f"• {top_cat} is the highest volume category")

    # Slowest subsystem
    ss_rt = df.groupby("subsystem")["time_to_resolve_hours"].mean().sort_values(ascending=False)
    insights.append(f"• {ss_rt.index[0]} has the longest resolution time at about {ss_rt.iloc[0]:.1f} hours")

    # Root cause for SLA breaches
    sla_df = df[df["sla_breached"] == "Yes"]
    if not sla_df.empty:
        rc = sla_df["root_cause"].value_counts(normalize=True).sort_values(ascending=False)
        insights.append(f"• {rc.index[0]} is responsible for around {rc.iloc[0] * 100:.1f}% of SLA breaches")

    # Financial impact leader
...
# PAGE TITLE
# --------------------------------------------------
st.title("Risk and Incident Analytics Dashboard")

st.markdown("This dashboard tracks operational incidents across systems, channels, categories, and regions.")
...
c5.metric("Repeat Incidents", f"{kpis['Repeat']}%")
...
with tab_overview:
    if filtered_df.empty:
        ...
        st.plotly_chart(px.line(daily, x="date", y="count", title="Incidents Over Time"), use_container_width=True)
...
with tab_fin:
    if filtered_df.empty:
        ...
        colA.plotly_chart(
            px.bar(fi_cat, x="category", y="financial_impact_usd", title="Financial Impact by Category"),
            use_container_width=True
        )
        ...
        colB.plotly_chart(
            px.bar(fi_ss, x="subsystem", y="financial_impact_usd", title="Financial Impact by Subsystem"),
            use_container_width=True
        )
        ...
        st.dataframe(
            top10[["incident_id", "date", "category", "severity_level", "subsystem", "financial_impact_usd"]]
        )
